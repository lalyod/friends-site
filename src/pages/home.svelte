<script>
  ...
  async function handleFetch() {
    try {
      fetchLoading = true;
      const { apps } = await fetchJSON(`/app/${BOT_ID}/status`);
      const { apps: logs } = await fetchJSON(`/app/${BOT_ID}/logs`);
  ...
  async function handleRestart() {
    try {
      loading = true;
      await fetchJSON(`/app/${BOT_ID}/restart`, { method: "PUT" });
    } finally {
      await handleFetch();
      loading = false;
    }
  ...
  async function handleStop() {
    try {
      loading = true;
      await fetchJSON(`/app/${BOT_ID}/stop`, { method: "PUT" });
    } finally {
      await handleFetch();
      loading = false;
    }
    ...
    <main class="px-4 lg:px-6">
      <div class="my-5 grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 gap-5">
        <Stat
          ...
          info={app?.last_restart ? `Online ${app?.last_restart || app.last_restart === "Offline" ? "0 hours" : app?.last_restart}`: ''}
        ...
        <Stat
          ...
          info={app
            ? parseFloat(app.memory?.replace("%", "")) > 50
              ? "Abnomal"
              : "Normal"
            : "Loading..."}
          detailInfo={app
      ...
      <div class="mockup-window border border-base-300 dark:bg-base-200">
        ...
        {:else}
          <pre
            class="max-h-96 mx-3 px-3 mb-3 pb-3 overflow-y-auto">{terminal}</pre>
        {/if}
      </div>
    </main>
  </Layout>
